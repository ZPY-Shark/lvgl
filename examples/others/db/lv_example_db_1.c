...
#if LV_USE_MSG && LV_USE_DB && LV_USE_SLIDER && LV_USE_LABEL && LV_BUILD_EXAMPLES
...
void lv_example_db_1(void)
{
    ...
    lv_db_subscribe_obj_to_value(db, &value, slider, NULL);
...
static void slider_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED) {
        value = limit_value(lv_slider_get_value(slider));
        lv_db_update_value(db, &value);
    }
